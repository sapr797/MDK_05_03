...
- [ ] Тест с дубликатами
- [ ] Тест со строками
- [ ] Тест со смешанными тиmarkdown
### find_duplicates (цель: 100%)
- [ ] Тест с пустым списком
- [ ] Тест без дубликатов
- [ ] Тест с дубликатами (числа)
- [ ] Тест со строками
- [ ] Тест со смешанными типами (числа и строки)
- [ ] Тест с `None` значениями
- [ ] Тест с булевыми значениями (`True`/`False`)
- [ ] Тест с дубликатами, сохраняющими порядок
- [ ] Тест с типом `TypeError` (входные данные не список)

### get_unique_elements (цель: 100%)
- [ ] Тест с пустым списком
- [ ] Тест без дубликатов (сохранение порядка)
- [ ] Тест с дубликатами
- [ ] Тест со строками
- [ ] Тест со смешанными типами
- [ ] Тест с `None` и булевыми значениями
- [ ] Тест с типом `TypeError`

### merge_dicts (цель: 100%)
- [ ] Тест стратегии `override` (без конфликта)
- [ ] Тест стратегии `override` (с конфликтом)
- [ ] Тест стратегии `skip` (с конфликтом)
- [ ] Тест стратегии `error` (без конфликта)
- [ ] Тест стратегии `error` (с конфликтом) — `KeyError`
- [ ] Тест стратегии `merge_lists` (оба значения — списки)
- [ ] Тест стратегии `merge_lists` (одно значение не список)
- [ ] Тест стратегии `merge_lists` (оба значения не списки)
- [ ] Тест с пустыми словарями
- [ ] Тест с `TypeError` (первый аргумент не словарь)
- [ ] Тест с `TypeError` (второй аргумент не словарь)
- [ ] Тест с неизвестной стратегией (`ValueError`)

### group_by_key (цель: 100%)
- [ ] Тест с пустым списком
- [ ] Тест с одной группой
- [ ] Тест с несколькими группами
- [ ] Тест с сохранением порядка элементов в группах
- [ ] Тест с ключами разных типов
- [ ] Тест с `TypeError` (входные данные не список)
- [ ] Тест с `TypeError` (элемент не словарь)
- [ ] Тест с `KeyError` (ключ отсутствует в элементе)

### flatten_list (цель: 100%)
- [ ] Тест с плоским списком
- [ ] Тест с одним уровнем вложенности
- [ ] Тест с несколькими уровнями вложенности
- [ ] Тест с пустыми вложенными списками
- [ ] Тест со смешанными типами данных
- [ ] Тест с глубиной разворачивания `depth = 0`
- [ ] Тест с глубиной разворачивания `depth = 1`
- [ ] Тест с глубиной разворачивания `depth = 2`
- [ ] Тест с глубиной разворачивания `depth >` фактической вложенности
- [ ] Тест с отрицательной глубиной (`depth = -1` — полное разворачивание)
- [ ] Тест с `TypeError` (входные данные не список)

### chunk_list (цель: 100%)
- [ ] Тест с `chunk_size = 1`
- [ ] Тест с `chunk_size =` больше длины списка
- [ ] Тест с `chunk_size =` равен длине списка
- [ ] Тест с пустым списком
- [ ] Тест с чанками разного размера (последний чанк меньше)
- [ ] Тест с `TypeError` (items не список)
- [ ] Тест с `TypeError` (chunk_size не целое число)
- [ ] Тест с `ValueError` (chunk_size = 0)
- [ ] Тест с `ValueError` (chunk_size отрицательный)

### count_frequencies (цель: 100%)
- [ ] Тест с пустым списком
- [ ] Тест с уникальными элементами
...
markdown
### Задание 3.4. Финальный запуск и анализ (15 минут)

```bash
# Финальный запуск со всеми опциями
pytest test_collection_manager_complete.py --cov=collection_manager --cov-report=term --cov-report=html --cov-report=xml --cov-branch --cov-fail-under=90

# Проверка, что покрытие >= 90%
Зафиксируйте результаты:
...
| Покрытие строк | ___% |
| Покрытие ветвей | ___% |
| Покрытие функций | ___% |
| Всего строк в модуле | ___ |
| Непокрытых строк | ___ |

### Покрытие по функциям

| Функция | Строк | Покрытие строк | Ветвей | Покрытие ветвей |
|:---|:---|:---|:---|:---|
| find_duplicates | | % | | % |
| get_unique_elements | | % | | % |
| merge_dicts | | % | | % |
...
### Причины непокрытых строк (если есть)
...
Опишите, как можно интегрировать проверку покрытия в CI/CD (GitHub Actions, GitLab CI):

_______________________________________________________________

_______________________________________________________________
Задание 3.5. Интеграция в CI/CD (10 минут)
...
jobs:
  test:
    ...
    - name: Run tests with coverage
      run: |
        pytest --cov=collection_manager --cov-report=xml --cov-report=term --cov-fail-under=85
...
Задание: Объясните, что делает каждый шаг в этом CI-пайплайне.

Карточка студента
text
ПР 3.9. ПОКРЫТИЕ КОДА. АНАЛИЗ COVERAGE
...
□ Установка pytest-cov (базовый)
□ Запуск тестов с coverage (базовый)
□ Анализ терминального отчёта (базовый)
□ Генерация HTML-отчёта (средний)
□ Выявление непокрытых строк (средний)
□ Улучшение тестов (средний)
□ Анализ покрытия ветвей (сложный)
□ Конфигурация coverage (сложный)
□ Достижение 90% покрытия (сложный)
□ CI/CD интеграция (сложный)

=== РЕЗУЛЬТАТЫ ===

Итоговое покрытие строк: _____%
Итоговое покрытие ветвей: _____%

Тестов написано: _____
Непокрытых строк осталось: _____
...
- test_collection_manager_base.py
- test_collection_manager_improved.py
- test_collection_manager_complete.py
...
2 (неудовлетворительно)	Любой	Coverage не запущен или нет отчёта
3 (удовлетворительно)	Базовый	Запуск coverage, терминальный отчёт, покрытие ≥ 50%
4 (хорошо)	Средний	HTML-отчёт, улучшение тестов, покрытие ≥ 80%
5 (отлично)	Сложный	Покрытие ветвей, конфигурация, покрытие ≥ 90%
Контрольные вопросы (для защиты)
Что такое покрытие строк и чем оно отличается от покрытия ветвей?

Как установить pytest-cov и запустить тесты с измерением покрытия?
...
Чем отличается --cov-report=term от --cov-report=term-missing?

Как проанализировать, какие ветви if/else не покрыты тестами?

Как интегрировать coverage в процесс разработки?

Следующее занятие: ПР 3.10 — Тестирование файлового ввода-вывода.
